...
struct KeyValueOffset {
    ...
    union {
        ...
        struct {
            block_size_t block_number : 36;
            numa_index_t numa_id : 4; // MAX NUMA count 16
            page_size_t page_number : 8;
    ...
    KeyValueOffset(const block_size_t block_number, const page_size_t page_number, const data_offset_size_t slot)
        : block_number{block_number}, numa_id{0}, page_number{page_number}, data_offset{slot} {}

    KeyValueOffset(const block_size_t block_number,  const numa_index_t numa_id, 
		    const page_size_t page_number, const data_offset_size_t slot)
        : block_number{block_number}, numa_id{numa_id}, page_number{page_number}, data_offset{slot} {}

    static KeyValueOffset Tombstone() {
        return KeyValueOffset{};
    }

    inline std::tuple<block_size_t, numa_index_t, page_size_t, data_offset_size_t> get_offsets() const {
        return {block_number, numa_id, page_number, data_offset};
    }
