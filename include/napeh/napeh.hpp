...
#define MAX_RUN_CLIENT (160)
#define DEFAULT_CHUNK_CNT (1024ul*1024ul*16ul)
#define FREE_SLOT_SYNC (0x13572468)
...
static constexpr uint8_t NUM_DIMMS = 60;
...
struct ViperConfig {
    ...
    size_t numa_cnt = 2; // default NUMA count
    size_t numa_swap_subset_cnt = 100; // default check NUMA swap for every 100 subset  
};
...
struct alignas(PAGE_SIZE) ViperPage {
    ...
    uint64_t pmem_free_slots_valid = 0;
    ...
    inline void unlock() {
     ...
     // memory lock /unlock
     inline bool lockm(std::atomic<version_lock_t> * mlock, bool blocking) {
        version_lock_t lock_value = mlock->load(LOAD_ORDER);
		int try_num = 0;
		
        lock_value &= UNLOCKED_BIT;
        while (!mlock->compare_exchange_weak(lock_value, lock_value + 1)) {
            lock_value &= UNLOCKED_BIT;
			try_num++;
            if (!blocking) { return false; }
        }
    ...
    inline void unlockm(std::atomic<version_lock_t> * mlock) {
        const version_lock_t current_version = mlock->load(LOAD_ORDER);
        version_lock_t new_version = (current_version + 1) % USED_BIT;
        new_version |= USED_BIT;
        new_version |= (current_version & CLIENT_BIT);
        mlock->store(new_version, STORE_ORDER);
    }
...
struct alignas(PAGE_SIZE) ViperPage<std::string, std::string> {
    ...
    // memory lcok and unlock for string
    inline bool lockm(std::atomic<version_lock_t> * mlock, bool blocking) {
        version_lock_t lock_value = mlock->load(LOAD_ORDER);
		int try_num = 0;
...
struct ViperBase {
    ...
    std::vector<ViperFileMapping> numa_v_mappings[8];
    std::vector<ViperFileMapping> v_mappings_0;
    std::vector<ViperFileMapping> v_mappings_1;
    std::vector<ViperFileMapping> v_mappings_2;
    std::vector<ViperFileMapping> v_mappings_3;
    std::vector<ViperFileMapping> v_mappings_4;
    std::vector<ViperFileMapping> v_mappings_5;
    std::vector<ViperFileMapping> v_mappings_6;
    std::vector<ViperFileMapping> v_mappings_7;
...
struct ViperInitData {
    ...
    std::vector<ViperFileMapping> numa_mappings;
    std::vector<ViperFileMapping> mappings_0;
    std::vector<ViperFileMapping> mappings_1;
    std::vector<ViperFileMapping> mappings_2;
    std::vector<ViperFileMapping> mappings_3;
    std::vector<ViperFileMapping> mappings_4;
    std::vector<ViperFileMapping> mappings_5;
    std::vector<ViperFileMapping> mappings_6;
    std::vector<ViperFileMapping> mappings_7;
...
class Viper {
  ...
  public:
    ...
    void reclaim();
    // free_slots and chunk locks pool in  DRAM, for each NUMA
    std::bitset<256>*  global_free_slots_0 = NULL;
    std::atomic<version_lock_t> * global_version_locks_0 = NULL;
    std::bitset<256>*  global_free_slots_1 = NULL;
    std::atomic<version_lock_t> * global_version_locks_1 = NULL;

    std::bitset<256>*  global_free_slots_2 = NULL;
    std::atomic<version_lock_t> * global_version_locks_2 = NULL;
    std::bitset<256>*  global_free_slots_3 = NULL;
    std::atomic<version_lock_t> * global_version_locks_3 = NULL;

    std::bitset<256>*  global_free_slots_4 = NULL;
    std::atomic<version_lock_t> * global_version_locks_4 = NULL;
    std::bitset<256>*  global_free_slots_5 = NULL;
    std::atomic<version_lock_t> * global_version_locks_5 = NULL;

    std::bitset<256>*  global_free_slots_6 = NULL;
    std::atomic<version_lock_t> * global_version_locks_6 = NULL;
    std::bitset<256>*  global_free_slots_7 = NULL;
    std::atomic<version_lock_t> * global_version_locks_7 = NULL;
	...
	void * run_queue;
      ...
      protected:
        ...
        bool put(const K& key,  V& value, bool delete_old);
	...
	numa_index_t  prefered_numa = 0;
        VPageBlock* v_block_;
        VPage* v_page_;
	// add some  META IN DRAM  
	std::bitset<256>* v_mem_free_slots = NULL;
	std::atomic<version_lock_t> * v_mem_version_lock = NULL;
    ...
    Client * get_run_client(int client_id, int is_write);
    Client * run_clients[MAX_RUN_CLIENT];
  ...
  protected:
    ...
    void update_mem_meta(Client * client);
    ...
    KVOffset get_new_block(int numa_id);
    void remove_client(Client* client);

    ViperFileMapping allocate_v_page_blocks(int numa_id);
    void add_v_page_blocks(ViperFileMapping mapping, int numa_id);
    void recover_database();
    void trigger_resize(int numa_id);
    ...
    std::vector<VPageBlock*> v_blocks_array[8];

    std::atomic<size_t> num_v_blocks_array[8];
    std::atomic<size_t> current_size_;
    std::atomic<size_t> reclaimable_ops_;
    std::atomic<offset_size_t> current_block_page_[8];
    ...
    uint32_t numa_cnt = 2; // default NUMA count
};
...
Viper<K, V>::Viper(ViperBase v_base, const std::filesystem::path pool_dir, const bool owns_pool, const ViperConfig v_config) :
    ...
    /*init numa one one , thus decrease pointer oprations ...*/

    numa_cnt = v_config.numa_cnt;
    for (int n = 9; n< numa_cnt; n++){
      current_block_page_[n] = 0;
    }
    if (v_base_.v_mappings_0.empty()) {
        throw new std::runtime_error("Need to have at least one memory section mapped.");
    }

    
    // init numa free_slots
    void * numa_free_slots = numa_alloc_onnode(sizeof(std::bitset<256>)*DEFAULT_CHUNK_CNT,0);
	if(numa_free_slots == NULL){
		printf("can not alloc numa_freeslots\n");
		exit(1);
	}
	global_free_slots_0 = new(numa_free_slots) std::bitset<256>[DEFAULT_CHUNK_CNT];
        if(numa_cnt >1){
    		numa_free_slots = numa_alloc_onnode(sizeof(std::bitset<256>)*DEFAULT_CHUNK_CNT,1);
		if(numa_free_slots == NULL){
			printf("can not alloc numa_freeslots 1\n");
			exit(1);
		}

		global_free_slots_1 = new(numa_free_slots) std::bitset<256>[DEFAULT_CHUNK_CNT];
	}
	if(numa_cnt >2){
    		numa_free_slots = numa_alloc_onnode(sizeof(std::bitset<256>)*DEFAULT_CHUNK_CNT,2);
		if(numa_free_slots == NULL){
			printf("can not alloc numa_freeslots 2\n");
			exit(1);
		}

		global_free_slots_2 = new(numa_free_slots) std::bitset<256>[DEFAULT_CHUNK_CNT];
	}
	if(numa_cnt >3){
    		numa_free_slots = numa_alloc_onnode(sizeof(std::bitset<256>)*DEFAULT_CHUNK_CNT,3);
		if(numa_free_slots == NULL){
			printf("can not alloc numa_freeslots 3\n");
			exit(1);
		}

		global_free_slots_3 = new(numa_free_slots) std::bitset<256>[DEFAULT_CHUNK_CNT];
	}
	if(numa_cnt >4){
    		numa_free_slots = numa_alloc_onnode(sizeof(std::bitset<256>)*DEFAULT_CHUNK_CNT,4);
		if(numa_free_slots == NULL){
			printf("can not alloc numa_freeslots 4\n");
			exit(1);
		}

		global_free_slots_4 = new(numa_free_slots) std::bitset<256>[DEFAULT_CHUNK_CNT];
	}
	if(numa_cnt >5){
    		numa_free_slots = numa_alloc_onnode(sizeof(std::bitset<256>)*DEFAULT_CHUNK_CNT,5);
		if(numa_free_slots == NULL){
			printf("can not alloc numa_freeslots 5\n");
			exit(1);
		}

		global_free_slots_5 = new(numa_free_slots) std::bitset<256>[DEFAULT_CHUNK_CNT];
	}
	if(numa_cnt >6){
    		numa_free_slots = numa_alloc_onnode(sizeof(std::bitset<256>)*DEFAULT_CHUNK_CNT,6);
		if(numa_free_slots == NULL){
			printf("can not alloc numa_freeslots 6\n");
			exit(1);
		}

		global_free_slots_6 = new(numa_free_slots) std::bitset<256>[DEFAULT_CHUNK_CNT];
	}
	if(numa_cnt >7){
    		numa_free_slots = numa_alloc_onnode(sizeof(std::bitset<256>)*DEFAULT_CHUNK_CNT,7);
		if(numa_free_slots == NULL){
			printf("can not alloc numa_freeslots 3\n");
			exit(1);
		}

		global_free_slots_7 = new(numa_free_slots) std::bitset<256>[DEFAULT_CHUNK_CNT];
	}

    // init numa  version_locks
    void * numa_version_locks = numa_alloc_onnode(sizeof(std::atomic<version_lock_t>)*DEFAULT_CHUNK_CNT,0);
	if(numa_version_locks == NULL){
		printf("can not alloc numa_version_locks\n");
		exit(1);
	}
	global_version_locks_0 = new(numa_version_locks) std::atomic<version_lock_t>[DEFAULT_CHUNK_CNT];
        if(numa_cnt >1){
		numa_version_locks = numa_alloc_onnode(sizeof(std::atomic<version_lock_t>)*DEFAULT_CHUNK_CNT,1);
		if(numa_version_locks == NULL){
			printf("can not alloc numa_version_locks 1\n");
			exit(1);
		}

		global_version_locks_1 = new(numa_version_locks) std::atomic<version_lock_t>[DEFAULT_CHUNK_CNT];
	}
	if(numa_cnt >2){
		numa_version_locks = numa_alloc_onnode(sizeof(std::atomic<version_lock_t>)*DEFAULT_CHUNK_CNT,2);
		if(numa_version_locks == NULL){
			printf("can not alloc numa_version_locks 2\n");
			exit(1);
		}

		global_version_locks_2 = new(numa_version_locks) std::atomic<version_lock_t>[DEFAULT_CHUNK_CNT];
	}
	if(numa_cnt >3){
		numa_version_locks = numa_alloc_onnode(sizeof(std::atomic<version_lock_t>)*DEFAULT_CHUNK_CNT,3);
		if(numa_version_locks == NULL){
			printf("can not alloc numa_version_locks 3\n");
			exit(1);
		}

		global_version_locks_3 = new(numa_version_locks) std::atomic<version_lock_t>[DEFAULT_CHUNK_CNT];
	}
	if(numa_cnt >4){
		numa_version_locks = numa_alloc_onnode(sizeof(std::atomic<version_lock_t>)*DEFAULT_CHUNK_CNT,4);
		if(numa_version_locks == NULL){
			printf("can not alloc numa_version_locks 4\n");
			exit(1);
		}

		global_version_locks_4 = new(numa_version_locks) std::atomic<version_lock_t>[DEFAULT_CHUNK_CNT];
	}
	if(numa_cnt >5){
		numa_version_locks = numa_alloc_onnode(sizeof(std::atomic<version_lock_t>)*DEFAULT_CHUNK_CNT,5);
		if(numa_version_locks == NULL){
			printf("can not alloc numa_version_locks 5\n");
			exit(1);
		}

		global_version_locks_5 = new(numa_version_locks) std::atomic<version_lock_t>[DEFAULT_CHUNK_CNT];
	}
	if(numa_cnt >6){
		numa_version_locks = numa_alloc_onnode(sizeof(std::atomic<version_lock_t>)*DEFAULT_CHUNK_CNT,6);
		if(numa_version_locks == NULL){
			printf("can not alloc numa_version_locks 6\n");
			exit(1);
		}

		global_version_locks_6 = new(numa_version_locks) std::atomic<version_lock_t>[DEFAULT_CHUNK_CNT];
	}
	if(numa_cnt >7){
		numa_version_locks = numa_alloc_onnode(sizeof(std::atomic<version_lock_t>)*DEFAULT_CHUNK_CNT,7);
		if(numa_version_locks == NULL){
			printf("can not alloc numa_version_locks 4\n");
			exit(1);
		}

		global_version_locks_7 = new(numa_version_locks) std::atomic<version_lock_t>[DEFAULT_CHUNK_CNT];
	}		

	
    
    for (ViperFileMapping mapping : v_base_.v_mappings_0) {
        add_v_page_blocks(mapping, 0);

    }
    v_base_.numa_v_mappings[0] = std::move(v_base_.v_mappings_0);
    if(numa_cnt > 1){
	    for (ViperFileMapping mapping : v_base_.v_mappings_1) {
            add_v_page_blocks(mapping, 1);
    	}
	v_base_.numa_v_mappings[1] = std::move(v_base_.v_mappings_1);

    }
    if(numa_cnt > 2){
	    for (ViperFileMapping mapping : v_base_.v_mappings_2) {
            add_v_page_blocks(mapping, 2);
    	}
	v_base_.numa_v_mappings[2] = std::move(v_base_.v_mappings_2);

    }
    if(numa_cnt > 3){
	    for (ViperFileMapping mapping : v_base_.v_mappings_3) {
            add_v_page_blocks(mapping, 3);
    	}
	v_base_.numa_v_mappings[3] = std::move(v_base_.v_mappings_3);

    }

   if(numa_cnt > 4){
	    for (ViperFileMapping mapping : v_base_.v_mappings_4) {
            add_v_page_blocks(mapping, 4);
    	}
	v_base_.numa_v_mappings[4] = std::move(v_base_.v_mappings_4);

    }
    if(numa_cnt > 5){
	    for (ViperFileMapping mapping : v_base_.v_mappings_5) {
            add_v_page_blocks(mapping, 5);
    	}
	v_base_.numa_v_mappings[5] = std::move(v_base_.v_mappings_5);

    }
    if(numa_cnt > 6){
	    for (ViperFileMapping mapping : v_base_.v_mappings_6) {
            add_v_page_blocks(mapping, 6);
    	}
	v_base_.numa_v_mappings[6] = std::move(v_base_.v_mappings_6);

    }
    if(numa_cnt > 7){
	    for (ViperFileMapping mapping : v_base_.v_mappings_7) {
            add_v_page_blocks(mapping, 7);
    	}
	v_base_.numa_v_mappings[7] = std::move(v_base_.v_mappings_7);
    ...
    for (int n = 0; n<numa_cnt; n++)
    	current_block_page_[n] = KVOffset{v_base.v_metadata->num_used_blocks.load(LOAD_ORDER), 0, 0}.offset;


	
   // init all run_clients now
   for(int n = 0; n< MAX_RUN_CLIENT; n++){
	    run_clients[n] = new Client(*this);
	    this->get_new_access_information(run_clients[n]);
	    run_clients[n]->client_id = n;
	    //client_vlocks[n] = USED_BIT; 
    }
...
Viper<K, V>::~Viper() {
    if (owns_pool_) {
        ...
        for (const ViperFileMapping& mapping : v_base_.v_mappings_0) {
            munmap(mapping.start_addr, mapping.mapped_size);
        }
	if(numa_cnt >1){
		for (const ViperFileMapping& mapping : v_base_.v_mappings_1) {
            		munmap(mapping.start_addr, mapping.mapped_size);
        	}
	}
	if(numa_cnt >2){
		for (const ViperFileMapping& mapping : v_base_.v_mappings_2) {
            		munmap(mapping.start_addr, mapping.mapped_size);
        	}
	}
	if(numa_cnt >3){
		for (const ViperFileMapping& mapping : v_base_.v_mappings_3) {
            		munmap(mapping.start_addr, mapping.mapped_size);
        	}
	}
	if(numa_cnt >4){
		for (const ViperFileMapping& mapping : v_base_.v_mappings_4) {
            		munmap(mapping.start_addr, mapping.mapped_size);
        	}
	}

	if(numa_cnt >5){
		for (const ViperFileMapping& mapping : v_base_.v_mappings_5) {
            		munmap(mapping.start_addr, mapping.mapped_size);
        	}
	}
	if(numa_cnt >6){
		for (const ViperFileMapping& mapping : v_base_.v_mappings_6) {
            		munmap(mapping.start_addr, mapping.mapped_size);
        	}
	}
	if(numa_cnt >7){
		for (const ViperFileMapping& mapping : v_base_.v_mappings_7) {
            		munmap(mapping.start_addr, mapping.mapped_size);
        	}
	}
...
void set_mappings(ViperInitData init_data, int numa_id,  std::vector<ViperFileMapping> mappings){

	//init_data.numa_mappings[numa_id] = std::move(mappings);

	
	switch(numa_id){
		case 0:
			init_data.mappings_0 = std::move(mappings);
			 break;
		case 1:
			init_data.mappings_1 = std::move(mappings);
			 break;
		case 2:
			init_data.mappings_2 = std::move(mappings);
			 break;
		case 3:
			init_data.mappings_3 = std::move(mappings);
			 break;
		case 4:
			init_data.mappings_4 = std::move(mappings);
			 break;

		case 5:
			init_data.mappings_5 = std::move(mappings);
			 break;
		case 6:
			init_data.mappings_6 = std::move(mappings);
			 break;

		case 7:
			init_data.mappings_7 = std::move(mappings);
			 break;		
 ...
 int init_file_pool(const std::string& pool_dir, uint64_t pool_size,
                             bool is_new_pool, ViperConfig v_config, const size_t block_size, ViperInitData init_data, int numa_id) {
    ...
    // only place meta file in numa pm0
    const std::filesystem::path meta_file = pool_dir + "/numa_pm0/meta";
    ...
    for (size_t chunk_num = 0; chunk_num < num_alloc_chunks; ++chunk_num) {
        std::filesystem::path data_file = pool_dir + "/numa_pm/" + std::to_string(numa_id) 
		+ "/data" + std::to_string(chunk_num);
    ...
    ::close(meta_fd);
    init_data.fd = -1;
    init_data.meta = metadata;
    set_mappings(init_data, numa_id, mappings);
    return 0;
    //return ViperInitData{ .fd = -1, .meta = metadata, .mappings = std::move(mappings) };
}
...
ViperBase Viper<K, V>::init_pool(const std::string& pool_file, uint64_t pool_size,
    ...
    if (is_file_based) {
	for (int numa_num = 0; numa_num < v_config.numa_cnt; numa_num++ ){
        	int ret = init_file_pool(pool_file, pool_size, is_new_pool, v_config, block_size, init_data, numa_num);
	}
    } else {
    ...
    return ViperBase{ .file_descriptor = init_data.fd, .is_new_db = is_new_pool,
   		      ...
   		      .v_mappings_0 = std::move(init_data.mappings_0), 
   		      .v_mappings_1 = std::move(init_data.mappings_1),
   		      .v_mappings_2 = std::move(init_data.mappings_2),
   		      .v_mappings_3 = std::move(init_data.mappings_3),
   		      .v_mappings_4 = std::move(init_data.mappings_4),
   		      .v_mappings_5 = std::move(init_data.mappings_5),
   		      .v_mappings_6 = std::move(init_data.mappings_6),
   		      .v_mappings_7 = std::move(init_data.mappings_7) };
}

template <typename K, typename V>
ViperFileMapping Viper<K, V>::allocate_v_page_blocks(int numa_id) {
    ...
    // TODO multiple NUMAs, not only numa_0
    v_base_.numa_v_mappings[numa_id].push_back(mapping);
    return mapping;
}

template <typename K, typename V>
void Viper<K, V>::add_v_page_blocks(ViperFileMapping mapping, int numa_id) {
    ...
    v_blocks_array[numa_id].reserve(v_blocks_.size() + num_blocks_to_map);
    is_v_blocks_resizing_.store(false, STORE_ORDER);
    for (block_size_t block_offset = 0; block_offset < num_blocks_to_map; ++block_offset) {
        v_blocks_array[numa_id].push_back(start_block + block_offset);
    }
    num_v_blocks_array[numa_id].store(v_blocks_array[numa_id].size(), STORE_ORDER);
}
...
template <typename K, typename V>
void Viper<K, V>::update_mem_meta(Client* client) {
	size_t block = client->v_block_number_;
        size_t page = client->v_page_number_;
	size_t index = block*NUM_DIMMS + page;
	switch(client->prefered_numa){
		case 0:
			client->v_mem_free_slots = &global_free_slots_0[index];
			client->v_mem_version_lock = &global_version_locks_0[index];
			break;
		case 1:
			client->v_mem_free_slots = &global_free_slots_1[index];
			client->v_mem_version_lock = &global_version_locks_1[index];
			break;
		case 2:
			client->v_mem_free_slots = &global_free_slots_2[index];
			client->v_mem_version_lock = &global_version_locks_2[index];
			break;
		case 3:
			client->v_mem_free_slots = &global_free_slots_3[index];
			client->v_mem_version_lock = &global_version_locks_3[index];
			break;
		case 4:
			client->v_mem_free_slots = &global_free_slots_4[index];
			client->v_mem_version_lock = &global_version_locks_4[index];
			break;
		case 5:
			client->v_mem_free_slots = &global_free_slots_5[index];
			client->v_mem_version_lock = &global_version_locks_5[index];
			break;
		case 6:
			client->v_mem_free_slots = &global_free_slots_6[index];
			client->v_mem_version_lock = &global_version_locks_6[index];
			break;
		case 7:
			client->v_mem_free_slots = &global_free_slots_7[index];
			client->v_mem_version_lock = &global_version_locks_7[index];
			break;
		default:
			client->v_mem_free_slots = &global_free_slots_0[index];
			client->v_mem_version_lock = &global_version_locks_0[index];
...
void Viper<K, V>::get_new_access_information(Client* client) {
    ...
    const KVOffset block_page{current_block_page_[client->prefered_numa].load(LOAD_ORDER)};
    if (block_page.block_number > resize_threshold_ * v_blocks_array[client->prefered_numa].size()) {
        trigger_resize(client->prefered_numa);
    }
...
void Viper<K, V>::get_block_based_access(Client* client) {
    ...
    if (!free_blocks_.try_dequeue(client_block)) {
        ...
        const KVOffset new_block = get_new_block(client->prefered_numa);
...
template <typename K, typename V>
KeyValueOffset Viper<K, V>::get_new_block(int numa_id) {
    offset_size_t raw_block_page = current_block_page_[numa_id].load(LOAD_ORDER);
    KVOffset new_offset{};
    block_size_t client_block;
    uint8_t numa_num = numa_id;
    do {
        ...
        while (client_block >= num_v_blocks_array[numa_id].load(LOAD_ORDER)) {
            asm("nop");
        }
        assert(new_block < v_blocks_array[numa_id].size());
        ...
        new_offset = KVOffset{new_block, numa_num, new_page, 0};
    } while (!current_block_page_[numa_id].compare_exchange_weak(raw_block_page, new_offset.offset));

    return KVOffset{raw_block_page};
}

template <typename K, typename V>
void Viper<K, V>::trigger_resize(int numa_id) {
    ...
    resize_thread_ = std::make_unique<std::thread>([this, numa_id] {
        DEBUG_LOG("Start resizing.");
	// TODO more NUMAs
        ViperFileMapping mapping = allocate_v_page_blocks(numa_id);
        add_v_page_blocks(mapping, numa_id);
...
template <typename K, typename V>
inline typename Viper<K, V>::Client *  Viper<K, V>::get_run_client(int client_id, int is_write) {
	//if(is_write)
	//	lock_client(client_id % MAX_VCLIENT_CNT);
	return run_clients[client_id % MAX_RUN_CLIENT];
}
...
template <typename K, typename V>
bool Viper<K, V>::Client::put(const K& key,  V& value, const bool delete_old) {
    //v_page_->lock();

    v_page_->lockm(v_mem_version_lock,true );
    // We now have the lock on this page
    std::bitset<256>* free_slots = v_mem_free_slots;
    const data_offset_size_t free_slot_idx = free_slots->_Find_first();

    if (free_slot_idx >= VPage::num_slots_per_page) {
        // Page is full. Free lock on page and restart.
	
	internal::pmem_memcpy_persist(&v_page_->free_slots, free_slots, sizeof (&v_page_->free_slots));
	v_page_->pmem_free_slots_valid = FREE_SLOT_SYNC;
	internal::pmem_persist( &v_page_->pmem_free_slots_valid,8);
        //v_page_->unlock();
	v_page_->unlockm(v_mem_version_lock );
        update_access_information();
        return put(key, value, delete_old);
    }

    // We have found a free slot on this page. Persist data.
    uint64_t * sign_ = const_cast<uint64_t*>(&key);
    value.set_signature((*sign_)<<32);
    ...
    const KVOffset kv_offset{v_block_number_, prefered_numa, v_page_number_, free_slot_idx};
    KVOffset old_offset;
...
bool Viper<K, V>::Client::put(const K& key, const V& value) {
    	V value1 {value};
	return put(key, value1, true);
}

template <typename K, typename V>
bool Viper<K, V>::Client::put(const K& key,  V& value) {
...
bool Viper<K, V>::Client::get(const K& key, V* value) {
	...
	/*TODO : the function below can be optimized by reduce some switch_case */
...
bool Viper<K, V>::Client::update(const K& key, UpdateFn update_fn) {
    ...
    while (true) {
        ...
        const auto [block, numa_index, page, slot] = kv_offset.get_offsets();
...
void Viper<K, V>::Client::free_occupied_slot(const KVOffset offset_to_delete, const K& key, const bool delete_offset) {
    const auto [block_number, numa_index, page_number, data_offset] = offset_to_delete.get_offsets();
...
void Viper<K, V>::Client::update_access_information() {
    ...
    v_page_->init();
    this->viper_.update_mem_meta(this);
}
...
Viper<K, V>::ReadOnlyClient::get_const_entry_from_offset(Viper::KVOffset offset) const {
    if constexpr (std::is_same_v<K, std::string>) {
        const auto[block, numa_index, page, data_offset] = offset.get_offsets();
        ...
        const auto[block, numa_index,page, slot] = offset.get_offsets();
...
inline bool Viper<K, V>::Client::get_value_from_offset(const KVOffset offset, V* value) {
    ...
    const VPage& v_page = this->viper_.v_blocks_array[numa_index][block]->v_pages[page];
    //std::atomic<version_lock_t>& page_lock = v_page.version_lock;

    switch(numa_index){
	case 0:
	{
    		version_lock_t lock_val =  
	    		this->viper_.global_version_locks_0[(uint64_t)block*NUM_DIMMS+page].load(LOAD_ORDER);
    		if (IS_LOCKED(lock_val)) {
        	return false;
    		}
		...
		if(!value->check_signature()){
    		...
    		return lock_val == ((this->viper_.global_version_locks_0[(uint64_t)block*NUM_DIMMS+page].load(LOAD_ORDER)));
		break;
	}
    	case 1:
	{
    		version_lock_t lock_val =  
	    		this->viper_.global_version_locks_1[(uint64_t)block*NUM_DIMMS+page].load(LOAD_ORDER);
    		if (IS_LOCKED(lock_val)) {
        	return false;
    		}
    		*value = v_page.data[slot].second;
		if(!value->check_signature()){
    		...
    		return lock_val == ((this->viper_.global_version_locks_1[(uint64_t)block*NUM_DIMMS+page].load(LOAD_ORDER)));
		break;
	}
	case 2:
	{
    		version_lock_t lock_val =  
	    		this->viper_.global_version_locks_2[(uint64_t)block*NUM_DIMMS+page].load(LOAD_ORDER);
    		if (IS_LOCKED(lock_val)) {
        	return false;
    		}
    		*value = v_page.data[slot].second;
		if(!value->check_signature()){
    		...
    		return lock_val == ((this->viper_.global_version_locks_2[(uint64_t)block*NUM_DIMMS+page].load(LOAD_ORDER)));
		break;
	}
    	case 3:
	{
    		version_lock_t lock_val =  
	    		this->viper_.global_version_locks_3[(uint64_t)block*NUM_DIMMS+page].load(LOAD_ORDER);
    		if (IS_LOCKED(lock_val)) {
        	return false;
    		}
    		*value = v_page.data[slot].second;
		if(!value->check_signature()){
    		...
    		return lock_val == ((this->viper_.global_version_locks_3[(uint64_t)block*NUM_DIMMS+page].load(LOAD_ORDER)));
		break;
	}
	case 4:
	{
    		version_lock_t lock_val =  
	    		this->viper_.global_version_locks_4[(uint64_t)block*NUM_DIMMS+page].load(LOAD_ORDER);
    		if (IS_LOCKED(lock_val)) {
        	return false;
    		}
    		*value = v_page.data[slot].second;
		if(!value->check_signature()){
    		...
    		return lock_val == ((this->viper_.global_version_locks_4[(uint64_t)block*NUM_DIMMS+page].load(LOAD_ORDER)));
		break;
	}
    	case 5:
	{
    		version_lock_t lock_val =  
	    		this->viper_.global_version_locks_5[(uint64_t)block*NUM_DIMMS+page].load(LOAD_ORDER);
    		if (IS_LOCKED(lock_val)) {
        	return false;
    		}
    		*value = v_page.data[slot].second;
		if(!value->check_signature()){
    		...
    		return lock_val == ((this->viper_.global_version_locks_5[(uint64_t)block*NUM_DIMMS+page].load(LOAD_ORDER)));
		break;
	}
	case 6:
	{
    		version_lock_t lock_val =  
	    		this->viper_.global_version_locks_6[(uint64_t)block*NUM_DIMMS+page].load(LOAD_ORDER);
    		if (IS_LOCKED(lock_val)) {
        	return false;
    		}
    		*value = v_page.data[slot].second;
		if(!value->check_signature()){
    		...
    		return lock_val == ((this->viper_.global_version_locks_6[(uint64_t)block*NUM_DIMMS+page].load(LOAD_ORDER)));
		break;
	}
    	case 7:
	{
    		version_lock_t lock_val =  
	    		this->viper_.global_version_locks_7[(uint64_t)block*NUM_DIMMS+page].load(LOAD_ORDER);
    		if (IS_LOCKED(lock_val)) {
        	return false;
    		}
    		*value = v_page.data[slot].second;
		if(!value->check_signature()){
    		...
    		return lock_val == ((this->viper_.global_version_locks_7[(uint64_t)block*NUM_DIMMS+page].load(LOAD_ORDER)));
		break;
	}
...
void Viper<K, V>::reclaim() {
    ...
    const block_size_t max_block = KVOffset{current_block_page_[0].load(LOAD_ORDER)}.block_number;
