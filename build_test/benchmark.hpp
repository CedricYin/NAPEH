...
struct BMRecord {
    ...
    uint64_t get_signature() const {
        return *(uint64_t*) (&(data[N-2]));
    }
    bool check_signature() const {
        return (*(uint64_t*) (&(data[N-2]))&VALID_SIGN == VALID_SIGN);
    }
    
    void set_signature(uint64_t signature)  {
        (*(uint64_t*) (&(data[N-2])) = signature | VALID_SIGN);
    }
...
// SignedValue  add the last 8-bytes for NAPEH_kv_signature
using SignedValue8 =   BMRecord<uint32_t, 4>;
using SignedValue16 =   BMRecord<uint32_t, 6>;
using SignedValue100 = BMRecord<uint32_t, 27>;
using SignedValue200 = BMRecord<uint32_t, 52>;
using SignedValue500 = BMRecord<uint32_t, 127>;
using SignedValue1000 = BMRecord<uint32_t, 252>;
